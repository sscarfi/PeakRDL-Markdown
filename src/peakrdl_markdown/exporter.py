...
class MarkdownExporter:  # pylint: disable=too-few-public-methods
    ...
    def _addrnode_info(node: AddressableNode):
        ...
        ret["Absolute Address"] = f"0x{int(node.absolute_address/div):X}"+" (in decimal "+f"{int(node.absolute_address/div):d})"
        ret["Base Offset"] = f"0x{int(node.raw_address_offset/div):X}"+" (in decimal "+f"{int(node.raw_address_offset/div):d})"
